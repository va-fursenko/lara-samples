...
/**
 * Контроллер работы с нашим фронтом. Тонкий. Без бизнес-логики. 
 * Реквест используется только для общей валидации. Без бизнес-логики, например, проверки возраста туристов или сопоставления 
 * типа документа туриста и страны поездки - это уже глубже.
 *
 * По факту отличается от контроллера API только тем, что использует для результата FrontFormatter-ы, 
 * чья логика подстраивается под просьбы фронта. А для API - соответственно, используем API-форматтеры, в которых можно учесть какие-то 
 * особенности или пожелания партнёров. На практике, впрочем API берётся за эталон, обычно под него партнёры подстраиваются, 
 * а вот с фронта новые пожелания приходят гораздо чаще. Прослойка форматтера позволяет не трогать всю бизнес-логику для их исполнения.
 */
